...
//   const baseHeight = 1; // Height above the track surface
...
//   console.log(
//     `Car ${carIndex}: Row ${row}, ${
//       isLeftLane ? "Left" : "Right"
//     } Lane, Position:`,
//     position
//   );
...
//         scene.add(car);
//         console.log(
//           `Added existing opponent car: ${id} at position:`,
//           car.position
//         );
//       }
//     }
...
//       console.log(
//         "Skipping newPlayer event: myId not set or same as player ID",
//         { myId, playerId: playerData.id }
//       );
//       return;
//     }
...
function addFoliage() {
  ...
  console.log(
    `Added ${numFoliage} foliage instances with ${
      Object.keys(foliageGroups).length
    } draw calls.`
  );
}
    ...
    if (child.material) {
      ...
      materials.forEach((mat, index) => {
        ...
        if (mat.isMeshStandardMaterial || mat.isMeshPhysicalMaterial) {
          console.log(
            `Modifying PBR material: ${mat.name || "Unnamed"} on mesh ${
              child.name || "Unnamed"
            }`
          );

          // --- Apply Reflection Properties ---
          // You might want to add checks here based on material names
          // if you set them up in Blender (e.g., 'CarPaint', 'Windows', 'Tires')
          // Example: if (mat.name.includes('Paint')) {
          mat.metalness = metallicValue;
          mat.roughness = roughnessValue;
          // } else if (mat.name.includes('Window')) {
          //     mat.metalness = 0.1;
          //     mat.roughness = 0.1;
          //     mat.transparent = true;
          //     mat.opacity = 0.3;
          // } else { // Default for other parts like tires
          //     mat.metalness = 0.1;
          //     mat.roughness = 0.8;
          // }

          // The environment map is automatically picked up from scene.environment
          // You usually don't need to set mat.envMap explicitly here unless
          // you want a *different* reflection map for this specific object.
          // mat.envMap = scene.environment;

          mat.needsUpdate = true; // Good practice when modifying material properties
          modifiedMaterial = true;
        } else {
          // If the material from the FBX is not PBR (e.g., MeshBasicMaterial),
          // reflections won't work well. Log a warning.
          console.warn(
            `Material '${mat.name || "Unnamed"}' on mesh '${
              child.name || "Unnamed"
          ...
          // Optionally, you could try replacing it here, but it's better to fix the export.
          // Example Replacement (use with caution):
          // const newMat = new THREE.MeshStandardMaterial({
          //     map: mat.map || null, // Try to preserve texture
...
function updateHUDPosition() {
  ...
  if (!playerCar || !trackMesh || !trackMesh.userData || !trackMesh.userData.trackCurve) {
      positionElement.textContent = `Position: - / -`;
      return;
  }
  ...
  if (numCurvePoints < 2) { // Need at least 2 points to calculate progress
       positionElement.textContent = `Position: - / -`;
       console.error("Track curve has less than 2 points, cannot calculate position.");
       return;
  }
  ...
  try {
      ...
      console.error("Error calculating player position parameter:", error, "Player Position:", playerCar.position);
      raceData.push({ id: myId, lapCount: playerCar.userData.lapCount || 0, progressU: 0, combinedProgress: playerCar.userData.lapCount || 0 });
  }
  ...
  for (const id in opponentCars) {
      ...
      if (opponentCar) {
          try {
              ...
              console.error("Error calculating opponent position parameter:", error, "Opponent ID:", id, "Position:", opponentCar.position);
              raceData.push({ id: id, lapCount: opponentCar.userData.lapCount || 0, progressU: 0, combinedProgress: opponentCar.userData.lapCount || 0 });
          }
      }
  }
  ...
  if (playerRank > 0) {
      ...
      if(totalPlayers > 0 && playerCar) { // Only warn if there should be a rank
          console.warn("Could not determine player rank. Player ID:", myId, "Race Data:", raceData);
      }
  }
}
// --- Utility Functions ---
...
function showRoomSelectionPopup() {
  ...
  document.getElementById("createRaceButton").addEventListener("click", handleCreateRace);
  document.getElementById("joinRaceButton").addEventListener("click", showJoinRacePopup);
}
...
function showJoinRacePopup() {
  ...
  document.getElementById("joinButton").addEventListener("click", handleJoinRace);
}

function handleJoinRace() {
  currentRoomCode  = document.getElementById("roomCodeInput").value.trim().toUpperCase();
  if (currentRoomCode ) {
    socket.emit("joinRoom", { roomCode:currentRoomCode, playerName:playerName  });
  } else {
    document.getElementById("joinError").textContent = "Please enter a room code.";
  }
}
...
async function init() {
  ...
  loadingScreen.style.display = "none";
  animate();
  setTimeout(()=>socket.emit("getUpdate", { roomCode:currentRoomCode  }),1000);
}
...
// //main.js
// ... (other code above handleControls)
  ...
  if (playerCar && trackMesh && trackMesh.userData) {
      ...
      if (trackCurve) {
          ...
          if (Math.abs(distanceFromCenter) > trackHalfWidth) {
              ...
              const boundaryPoint = closestPointOnCurve.clone().add(
                  normal.clone().multiplyScalar(Math.sign(distanceFromCenter) * trackHalfWidth)
              );
      ...
      if (trackData.finishLineCenter && trackData.finishLineTangent) {
          ...
          const vectorToCenter = finalCarPos.clone().sub(trackData.finishLineCenter);
          const finishLineNormal = new THREE.Vector3(
              -trackData.finishLineTangent.z, 0, trackData.finishLineTangent.x
          ).normalize();
          const side = vectorToCenter.dot(finishLineNormal);
          let crossedFinishLineBackwardsOnLapZero = false;

          if (playerCar.userData.lapCount === 0 && playerCar.userData.prevSide !== null && !completed) {
              if (playerCar.userData.prevSide > 0 && side < 0) {
                  console.log("Attempted to cross start line backwards on lap 0. Preventing.");
                  ...
                  const revertedVectorToCenter = previousPosition.clone().sub(trackData.finishLineCenter);
                  side = revertedVectorToCenter.dot(finishLineNormal);
              }
          }

          if (playerCar.userData.prevSide !== null && !completed && !crossedFinishLineBackwardsOnLapZero) {
              if (playerCar.userData.prevSide < 0 && side > 0) {
                  playerCar.userData.lapCount++;
                  console.log(`Lap completed! Total laps: ${playerCar.userData.lapCount}`);
                  socket.emit("lapCompleted", { id: myId, lapCount: playerCar.userData.lapCount, roomCode: currentRoomCode });
...
// --- Remaining Functions (Unchanged Except for handleControls) ---
// setupSkybox, setupScene, setupLighting, createGround, createTrack,
// preloadFoliageTextures, addFoliage, preloadGrassTexture, addGrass,
// loadCarAssets, createCar, addOpponent, removeOpponent, setupCamera,
// updateCamera, setupControls, handleKeyDown, handleKeyUp, updateHUD,
// updateHUDPosition, showMessage, animate, onWindowResize remain unchanged